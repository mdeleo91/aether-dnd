...
export default function NpcCard({ card, onData }) {
  ...
  const generate = async () => {
    ...
    if (res.data?.name) onData(card.id, { npc: res.data })
  }
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-2">
        ...
        />
        <button onClick={generate} disabled={busy} title="Regenerate with AI" className="shrink-0 rounded-md border border-amethyst-400/40 px-2 py-0.5 text-[10px] text-amethyst-100 transition hover:bg-amethyst-400/10 disabled:opacity-50">
          {busy ? '…' : '↻ AI'}
        </button>
      </div>
      <input
      ...
      />
      {state && state !== 'demo' && <p className="mt-1 text-[10px] text-red-300">{state}</p>}
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 px-2 py-6 text-center">
      ...
      {state === 'demo' ? (
        ...
        <p className="text-xs text-red-300">{state}</p>
      ) : (
      ...
      >
        <Sparkles size={14} /> {busy ? 'Generating…' : `Generate ${label}`}
      </button>
    </div>
  )
}
